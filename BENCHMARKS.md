...
All timing uses Node.js `performance.now()` — monotonic, sub-millisecond resolution, unaffected by system clock adjustments.
...
After all queries complete, results are printed in order with a ✓/✗ status, latency in ms, and the query text — making it easy to spot outliers at a glance:
