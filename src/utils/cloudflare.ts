...
export async function handleCloudflare(page: Page): Promise<boolean> {
  ...
  logger.warn(
    `Cloudflare challenge detected! Engaging Vision-based bypass with ${config.ollamaVisionModel}...`
  )
    ...
    const pressure =
      attempt === 1
        ...
        : attempt === 2
          ...
          : 'CRITICAL: Return ONLY the JSON array. NO TEXT, NO COMMENTS.'

    const prompt = `Identify the exact pixel coordinates (x, y) of the "Verify you are human" checkbox of this given Cloudflare challenge screenshot. Estimate the three positions which look most likeably like the checkbox where the click must happen to resolve the captcha.
    The image is 1920x1080.
    ${pressure}
    Return ONLY a JSON array of objects with the correct coordinates:
  ...
  throw new Error(
    'Cloudflare bypass exhausted all retries. Failing fast to prevent hanging/blacklisting.'
  )
}
