...
const applicationConfigurationSchema = z.object({
  ...
  enableVectorSearch: z.string().optional().transform((value) => value === 'true'),
  headless: z.preprocess((value) => {
...
function parseConfigurationFromEnvironment(): Config {
  ...
  if (!validationResult.success) {
    validationResult.error.issues.forEach((validationIssue) => {
      ...
      const environmentVariableName = configurationFieldPath.replace(/[A-Z]/g, (letter) => `_${letter.toLowerCase()}`).toUpperCase()
      errorBus.emitError(`Configuration error: ${environmentVariableName} - ${validationIssue.message}`)
    })
    process.exit(1)
  }
