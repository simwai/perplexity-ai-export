...
const configSchema = z.object({
  ...
  aiProvider: z.enum(['ollama', 'openai-compatible']),
  aiEmbedProvider: z.enum(['ollama', 'openai-compatible']),
  aiBaseUrl: z.string().url(),
  aiApiKey: z.string().optional(),
  aiModel: z.string().min(1),
  aiEmbedModel: z.string().min(1),
...
function parseEnvConfig(): Config {
  ...
  const rawConfig = {
    ...
    aiProvider: (process.env['AI_PROVIDER'] || 'ollama') as 'ollama' | 'openai-compatible',
    aiEmbedProvider: (process.env['AI_EMBED_PROVIDER'] || 'ollama') as 'ollama' | 'openai-compatible',
    aiBaseUrl: process.env['AI_BASE_URL'] || (process.env['AI_API_KEY'] ? 'https://api.openai.com/v1' : (process.env['OLLAMA_URL'] || DEFAULT_OLLAMA_URL)),
    aiApiKey: process.env['AI_API_KEY'],
    aiModel: process.env['AI_MODEL'] || process.env['OLLAMA_MODEL'] || 'llama3.1',
    aiEmbedModel: process.env['AI_EMBED_MODEL'] || process.env['OLLAMA_EMBED_MODEL'] || 'nomic-embed-text',
    ...
    hydeMode: (process.env['HYDE_MODE'] || 'supplement') as 'off' | 'fusion' | 'supplement',
