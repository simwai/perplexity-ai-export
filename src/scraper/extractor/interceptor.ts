      ...
      webPage.on('response', async (webResponse) => {
        ...
        try {
          ...
          if (!validationResult.success) {
            ...
            const newEntriesToAppend = Array.isArray(validatedData)
              ? validatedData
              : validatedData.entries
            accumulatedApiEntries.push(...newEntriesToAppend)

            const hasMorePagesToCapture =
              !Array.isArray(validatedData) && validatedData.collection_info?.has_next_page
