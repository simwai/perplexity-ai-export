...
export class DataParser {
  ...
  parse(
  ...
  ): { entries: any[]; meta: any; hash: string } | null {
    const normalizedEntries = this.normalizeApiData(rawApiData, conversationUrl)
    const validationResult = z
      .array(DataParser.ConversationEntrySchema)
      .nonempty()
      .safeParse(normalizedEntries)

    if (!validationResult.success) {
      const areEntriesEmpty = normalizedEntries.length === 0
      if (areEntriesEmpty) {
        this.apiDiagnosticsWriter
    ...
    const contentIntegrityHash = createHash('sha256')
      .update(stringify(validatedEntries))
      .digest('hex')
    ...
    return {
      ...
      meta: {
        ...
        spaceName:
          firstEntryInConversation.collection_info?.title ??
          rawApiData?.collection_info?.title ??
          'General',
        timestamp: new Date(
          firstEntryInConversation.updated_datetime ?? rawApiData?.updated_datetime ?? new Date()
        ),
      },
    }
  }
  ...
  private normalizeApiData(rawApiData: any, conversationUrl: string): any[] {
    ...
    this.apiDiagnosticsWriter
      .writeFailure({ url: conversationUrl, errorType: 'unknown_shape' })
      .catch(() => {})
    return []
  }
}
