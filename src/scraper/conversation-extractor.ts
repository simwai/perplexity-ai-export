...
export class ConversationExtractor {
  ...
  constructor(private readonly applicationConfig: Config, private readonly browserContext: BrowserContext) {
  ...
  async extract(conversationUrl: string): Promise<ExtractedConversation> {
    ...
    try {
      ...
      return errorBus.raiseError(`Failed to create new page for ${conversationUrl}`, pageCreationError)
    }

    const captureApiResponsePromise = this.apiInterceptor.capture(conversationPage, this.currentTimeoutMilliseconds)
    ...
    try {
      ...
      return {
        ...
        formattedMarkdownContent: this.markdownFormatter.format(parsedDataResult!.entries, parsedDataResult!.meta.title)
      }
    } catch (extractionError) {
      const isExpectedError = extractionError instanceof Error &&
