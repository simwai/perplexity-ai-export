...
async function runPerformanceBenchmark(): Promise<void> {
  ...
  const benchmarkRunResults: { queryText: string; durationMilliseconds: number; isFailure: boolean }[] = []
  ...
  for (let queryIndex = 0; queryIndex < TEST_BENCHMARK_QUERIES.length; queryIndex++) {
    ...
    try {
      ...
      errorBus.emitError('Benchmark query failed', queryExecutionError, { query: currentBenchmarkQuery })
    }
  ...
  const averageLatencyMilliseconds = successfulBenchmarkRuns.length > 0
  ...
  benchmarkRunResults.forEach((runResult, index) => {
    ...
    logger.info(`  ${statusSuccessSymbol} [${index + 1}] ${runResult.durationMilliseconds}ms — ${runResult.queryText}`)
  })
