...
export class RagOrchestrator {
  ...
  constructor(applicationConfig: Config) {
    ...
    this.hybridRetriever = new HybridRetriever(applicationConfig, conversationVectorStore, ripgrepSearchEngine)
  ...
  async answerQuestion(userQuestion: string): Promise<void> {
    try {
      ...
      const rerankedSearchResults = await this.executeCrossEncoderReranking(userQuestion, rawSearchResults)
      ...
      const verificationFeedback = await this.responseSynthesizer.verifyQuality(userQuestion, finalGeneratedAnswer)
      ...
      errorBus.emitError('Mightiest RAG pipeline failed', orchestrationError, { question: userQuestion })
    }
  }
  ...
  private async executeCrossEncoderReranking(
    ...
    try {
      ...
      for (let currentBatchOffset = 0; currentBatchOffset < searchResults.length; currentBatchOffset += RERANKING_BATCH_SIZE) {
        const currentBatch = searchResults.slice(currentBatchOffset, currentBatchOffset + RERANKING_BATCH_SIZE)
        const inputPairsForEncoder = currentBatch.map((result) => [userQuestion, (result.meta['snippet'] as string) || ''])

        const tokenizedEncoderInputs = await tokenizer(inputPairsForEncoder.map((pair) => pair[0]), {
        ...
        const batchLogitScores: number[] = Array.from(modelOutputPrediction.logits.data as Float32Array)
  ...
  private displaySourceProvenance(extractedResearchFacts: ExtractedFact[]): void {
    const uniqueSourceDocumentTitles = new Set(extractedResearchFacts.map((fact) => fact.sourceDocumentTitle))
