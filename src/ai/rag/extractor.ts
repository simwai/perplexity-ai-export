...
export class FactExtractor {
  ...
  async extractFacts(
    ...
    const poolSizeLimit = isExhaustiveStrategy
      ? MAXIMUM_NODES_FOR_EXHAUSTIVE
      : MAXIMUM_NODES_FOR_PRECISE
    ...
    for (
      ...
      currentBatchOffset < candidateNodesPool.length;
      currentBatchOffset += ANALYSIS_BATCH_SIZE
    ) {
      const currentBatchNumber = Math.floor(currentBatchOffset / ANALYSIS_BATCH_SIZE) + 1
      const currentResultsBatch = candidateNodesPool.slice(
        currentBatchOffset,
        currentBatchOffset + ANALYSIS_BATCH_SIZE
      )

      logger.info(
        `Analyzing history snippets... batch ${currentBatchNumber} of ${totalBatchesToProcess}`
      )

      const batchContextText = currentResultsBatch
        .map(
          (result, indexWithinBatch) =>
            `[Node ${currentBatchOffset + indexWithinBatch}] ${result.meta['title']}: ${result.meta['snippet']}`
        )
        .join('\n\n')
        ...
        for (const factEntry of extractedFactsList) {
          ...
          extractedResearchFindings.push({
            ...
            conversationThreadTitle:
              factEntry.thread || originalSourceNode?.meta['title'] || 'Unknown',
          })
        }
  ...
  private extractJsonArrayFromResponse(responseText: string): any[] {
    ...
    if (regexMatchResult?.[0]) {
      try {
        ...
        errorBus.emitError('Failed to parse researcher JSON', parsingError, {
