...
describe('ConversationExtractor Hashing (Unit)', () => {
  ...
  it('should generate the same hash for identical entries', () => {
    const entries = [
      { id: '1', query_str: 'test', blocks: [{ markdown_block: { answer: 'abc' } }] },
    ]
    const data = { entries }
  ...
  it('should generate different hashes for different entries', () => {
    ...
    const entries2 = [
      { id: '1', query_str: 'test', blocks: [{ markdown_block: { answer: 'def' } }] },
    ]
    const res1 = (extractor as any).dataParser.parse(
      { entries: entries1 },
      'http://test.com/search/1'
    )
    const res2 = (extractor as any).dataParser.parse(
      { entries: entries2 },
      'http://test.com/search/1'
    )
