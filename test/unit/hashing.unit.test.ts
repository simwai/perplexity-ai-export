...
describe('ConversationExtractor Hashing (Unit)', () => {
  ...
  it('should generate the same hash for identical entries', () => {
    ...
    const data = { entries }
    const res1 = (extractor as any).parser.parse(data, 'http://test.com/search/1')
    const res2 = (extractor as any).parser.parse(data, 'http://test.com/search/1')
  ...
  it('should generate different hashes for different entries', () => {
    ...
    const res1 = (extractor as any).parser.parse({ entries: entries1 }, 'http://test.com/search/1')
    const res2 = (extractor as any).parser.parse({ entries: entries2 }, 'http://test.com/search/1')
  ...
  it('should be stable regardless of key order in entries', () => {
    ...
    const res1 = (extractor as any).parser.parse(data1, 'http://test.com/search/1')
    const res2 = (extractor as any).parser.parse(data2, 'http://test.com/search/1')
    expect(res1.hash).toBe(res2.hash)
  })
})
