...
import { joinFromRoot } from '../../src/utils/paths.js'
...
describe('ApiDiagnosticsWriter (Unit)', () => {
  ...
  it('should write diagnostic entry to jsonl file when debug is true', async () => {
    ...
    expect(fs.mkdir).toHaveBeenCalledWith(joinFromRoot('debug'), { recursive: true })
    expect(fs.appendFile).toHaveBeenCalledWith(
      joinFromRoot('debug', 'api-diagnostics.jsonl'),
