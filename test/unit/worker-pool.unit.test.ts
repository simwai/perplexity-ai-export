...
describe('WorkerPool Skip Logic (Unit)', () => {
  ...
  beforeEach(() => {
    ...
    mockBrowser = { newContext: vi.fn().mockResolvedValue({ close: vi.fn() }) } as unknown as Browser
  ...
  it('should skip file write if hash matches', async () => {
    ...
    await (pool as any).performConversationExtraction(worker, { conversationMetadata: { id: 'thread-1', url: 'url' }, currentAttemptCount: 0 })
