...
describe('Export to Search E2E Flow', () => {
  ...
  it('should complete the full flow from export to semantic search', async () => {
    ...
    await browserContext.route('**/rest/thread/list_ask_threads**', async (route) => {
      const mockedThreads = [
        { uuid: 'refactor-tips', slug: 'refactor-tips', title: 'Refactoring Tips', total_threads: 1 }
      ]
      await route.fulfill({ status: 200, contentType: 'application/json', body: JSON.stringify(mockedThreads) })
    })
    ...
    await browserContext.route('**/rest/thread/refactor-tips', async (route) => {
      ...
      await route.fulfill({ status: 200, contentType: 'application/json', body: JSON.stringify(threadData) })
    })
    ...
    const exportHandler = new ExportHandler(mockApplicationConfig, checkpointManager, searchOrchestrator)
    ...
    vi.spyOn(WorkerPool.prototype, 'initialize').mockImplementation(async function(this: any) {
        ...
        for (let i = 0; i < (this as any).applicationConfig.parallelWorkers; i++) {
          (this as any).activeWorkers.push({
            workerId: i,
            conversationExtractor: new (await import('../../src/scraper/conversation-extractor.js')).ConversationExtractor((this as any).applicationConfig, browserContext),
    ...
    const exportFile = join(mockApplicationConfig.exportDir, 'General', 'Refactoring_Tips (refactor-tips).md')
