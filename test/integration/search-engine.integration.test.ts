...
describe('Search Engine Integration', () => {
  ...
  it('should successfully index and search using VectorStore (Ollama mocked)', async () => {
    const embedSpy = vi.spyOn(OllamaClient.prototype, 'embed').mockImplementation(async (texts: string[]) => {
  ...
  it('should coordinate searches correctly via SearchOrchestrator', async () => {
    ...
    const vectorSearchSpy = vi.spyOn(VectorStore.prototype, 'search').mockResolvedValue([
    ...
    await orchestrator.search('this is a very long query about refactoring', 'auto', { pattern: 'this is a very long query about refactoring' })
